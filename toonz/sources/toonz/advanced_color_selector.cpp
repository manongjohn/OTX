...
QColor tPixelToQColor(TPixelRGBM32 pix) { return {pix.r, pix.g, pix.b, pix.m}; }
...
public:
  ...
  void initialize(TPanel *panel) override {
    ...
    QObject::connect(
        wheel, &AdvancedColorSelector::colorChanged,
        [palette_handle](QColor c) {
          auto palette    = palette_handle->getPalette();
          auto styleIndex = palette_handle->getStyleIndex();
          if (!palette || styleIndex < 0) return;
          if (palette->getStyle(styleIndex)->getMainColor() ==
              qColorToTPixel(c))
            return;
    ...
    auto update_wheel = [palette_handle, wheel]() {
      ...
      if (!palette || styleIndex < 0) return;
      wheel->setColor(
          tPixelToQColor(palette->getStyle(styleIndex)->getMainColor()));
      wheel->saveToHistory();
    };
    QObject::connect(palette_handle, &TPaletteHandle::colorStyleSwitched,
                     update_wheel);
    QObject::connect(palette_handle, &TPaletteHandle::colorStyleChanged,
                     update_wheel);
    QObject::connect(palette_handle, &TPaletteHandle::paletteSwitched,
                     update_wheel);
...
OpenFloatingPanel openAdvancedColorSelectorCommand(
    "MI_OpenAdvancedColorSelector", "AdvancedColorSelector",
