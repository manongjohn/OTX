...
int TVariantPath::compare(const TVariantPath &a, int beginA,
...
void TVariant::setParentForChilds() {
  if (m_type == List) {
...
void TVariant::resize(int size) {
  setType(List);
  int prevSize = (int)m_list.size();
  if (prevSize == size) return;
  m_list.resize(size);
  if (prevSize < size)
    for (TVariantList::iterator i = m_list.begin() + prevSize;
         i != m_list.end(); ++i)
      i->setParent(*this);
  touch();
}
...
void TVariant::insert(int index, const TVariant &v) {
...
void TVariant::remove(int index) {
...
TVariant &TVariant::operator[](int index) {
  setType(List);
  assert(index >= 0);
  int prevSize = (int)m_list.size();
  if (index >= prevSize) {
...
TVariant &TVariant::operator[](const TStringId &field) {
  setType(Map);
...
bool TVariant::remove(const TStringId &field) {
...
const TVariant &TVariant::byPath(const TVariantPath &path, int begin,
                                 int end) const {
...
TVariant &TVariant::byPath(const TVariantPath &path, int begin, int end) {
  if ((int)path.size() <= begin || begin >= end) return *this;
  return (*this)[path[begin]].byPath(path, begin + 1, end);
}
...
int TVariant::getParentPathSize(const TVariant &parent) const {
  int ac = 0;
...
bool TVariant::getParentPath(TVariantPath &outPath,
                             const TVariant &parent) const {
  if (!m_parent) {
    outPath.clear();
    return false;
  }
...
bool TVariant::getChildPathEntry(const TVariant &child,
                                 TVariantPathEntry &outEntry) const {
  for (const TVariant *a = &child; a->parent(); a = a->parent())
    if (a->parent() == this) {
      outEntry = a->parentPathEntry();
      return true;
    }
...
bool TVariant::isChildOf(const TVariant &other) const {
...
bool TVariant::isChildOrEqual(const TVariant &other) const {
...
const TVariant *TVariant::findCommonParent(const TVariant &other) const {
...
size_t TVariant::getMemSize() const {
...
void TVariant::toStream(std::ostream &stream, bool pretty, int level) const {
  struct Writer {
    const TVariant &data;
    std::ostream &stream;
    bool pretty;
    ...
    void writeTab(int level) {
      if (pretty)
        for (int i = 2 * level; i; --i) stream << " ";
    }
    void writeChar(char c) { stream.put(c); }
    void writeWord(const char *word) { stream << word; }
    ...
    void write() {
      switch (data.getType()) {
      case Bool:
        writeWord(data.getBool() ? "true" : "false");
        break;
      case Double:
        writeDouble(data.getDouble());
        break;
      case String:
...
void TVariant::fromStream(std::istream &stream, int *currentRow,
                          int *currentCol) {
  struct Reader {
    TVariant &data;
    std::istream &stream;
    ...
    void warning(const std::string &msg) {
      std::cerr << "TVariant load:" << row << ":" << col << ": " << msg
                << std::endl;
    }
    void error(const std::string &msg) {
      throw TVariantSyntaxException(row, col, msg);
    }
    void error() { error("cannot recognize type of data"); }
    ...
    void readHexUnicode(std::string &str) {
      ...
      if (code == 0) {
        warning("\\u token with zero code");
        return;
      }
    ...
    void read() {
      skipSpaces();
      if (!readNull() && !readBool() && !readDouble() && !readString() &&
          !readList() && !readMap())
        error();
    }
  };
  ...
  reset();
  int row = 1, col = 1;
  Reader(*this, stream, (currentRow ? *currentRow : row),
...
std::string TVariant::toString(bool pretty, int level) const {
...
void TVariant::fromString(const std::string &str, int *currentRow,
                          int *currentCol) {
