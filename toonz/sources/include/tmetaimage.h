...
typedef TSmartPointerT<TMetaObject>
    TMetaObjectP;                           //!< smart pointer to TMetaObject
typedef TMetaObjectP::Holder TMetaObjectH;  //!< smart holder of TMetaObject
typedef TMetaObjectP::Const
    TMetaObjectPC;  //!< smart pointer to constant TMetaObject
typedef std::vector<TMetaObjectP> TMetaObjectList;
typedef TConstArrayWrapperT<TMetaObjectPC, TMetaObjectP>
    TMetaObjectListCW;  // TMetaObjectListConstWrapper
...
public:
  explicit TMetaObject(const TStringId &typeName = TStringId(),
                       const TVariant &data      = TVariant());
  explicit TMetaObject(const std::string &typeName,
                       const TVariant &data = TVariant());
  ~TMetaObject();

  void setType(const TStringId &name);
  inline void setType(const std::string &name) { setType(TStringId(name)); }
  inline void resetType() { setType(TStringId()); }

  inline const TMetaObjectType *getTypeDesc() const { return m_typeDesc; }
  inline const TStringId &getType() const { return m_typeLink->first; }
  inline const std::string &getTypeName() const { return getType().str(); }
  inline const TVariant &data() const { return m_data; }
  inline TVariant &data() { return m_data; }
...
public:
  ...
  static void registerType(
      const TStringId &name,
      const TMetaObjectType &type);  //!< register new or add alias
  static void unregisterType(
      const TStringId &name);  //!< unregister single alias
  static void unregisterType(
      const TMetaObjectType &type);  //!< unregister all aliases
...
protected:
  ...
  public:
    ...
    explicit LockEvents(TMetaObjectHandler &owner) : owner(owner) {
      ++owner.m_locks;
    }
...
public:
  TMetaObjectHandler(TMetaObject &object)
      : m_object(object), m_typeDesc(*m_object.getTypeDesc()) {
    assert(m_object.getTypeDesc());
  }
  virtual ~TMetaObjectHandler() {}

  inline const TMetaObjectType &getTypeDesc() const { return m_typeDesc; }
  inline const TStringId &getType() const { return getTypeDesc().name; }
  inline const std::string &getTypeName() const { return getType().str(); }
  inline const TMetaObject &object() const { return m_object; }
  inline TMetaObject &object() { return m_object; }
  inline const TStringId &getAlias() const { return object().getType(); }
  inline const std::string &getAliasName() const { return getAlias().str(); }
  inline const TVariant &data() const { return object().data(); }
  inline TVariant &data() { return object().data(); }
...
public:
  ...
  public:
    ...
    const TMetaImage &image() const { return m_image; }
    const TMetaObjectListCW &get() const { return m_objects; }
    const TMetaObjectListCW &operator*() const { return get(); }
    const TMetaObjectListCW *operator->() const { return &get(); }
  };
  ...
  public:
    Writer(TMetaImage &image) : QWriteLocker(&image.m_rwLock), m_image(image) {}
    TMetaImage &image() const { return m_image; }
    TMetaObjectList &get() const { return m_image.m_objects; }
    TMetaObjectList &operator*() const { return get(); }
    TMetaObjectList *operator->() const { return &get(); }
  };
