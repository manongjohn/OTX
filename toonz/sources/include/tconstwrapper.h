...
public:
  ...
  private:
    ...
    explicit reverse_iterator(const src_value_type *pointer)
        : pointer(pointer) {}
  ...
  public:
    ...
    reverse_iterator operator+(ptrdiff_t diff) const {
      return reverse_iterator(pointer - diff);
    }
    reverse_iterator operator-(ptrdiff_t diff) const {
      return reverse_iterator(pointer + diff);
    }
    ...
    reverse_iterator operator++(int) {
      --pointer;
      return reverse_iterator(pointer + 1);
    }
    reverse_iterator operator--(int) {
      ++pointer;
      return reverse_iterator(pointer - 1);
    }
    reverse_iterator operator+=(ptrdiff_t diff) {
      pointer -= diff;
      return *this;
    }
    reverse_iterator operator-=(ptrdiff_t diff) {
...
public:
  TConstArrayWrapperT() : m_begin(), m_end(), m_rbegin(), m_rend(), m_size() {}
  ...
  TConstArrayWrapperT &operator=(const TConstArrayWrapperT &other) {
    set(other.m_begin, other.m_size);
    return *this;
  }
  ...
  reverse_iterator rbegin() const { return reverse_iterator(m_rbegin); }
  reverse_iterator rend() const { return reverse_iterator(m_rend); }
  const value_type &at(size_t index) const {
    assert(index < m_size);
    return *cast(m_begin + index);
  }
