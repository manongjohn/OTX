...
public:
  ...
  inline TTimerTicks ticks() const
    { return m_ticks; }
  inline void releaseAll(TTimerTicks ticks) {
    keyReleaseAll(ticks);
    buttonReleaseAllAtAllDevices(ticks);
  }
  ...
  inline void keyReleaseAll(TTimerTicks ticks) {
    touch(ticks);
    keyHistory()->released_all(m_ticks);
  }
  ...
  // take holder of keys states hystory
  ...
  inline void buttonEvent(bool press, Button button, TTimerTicks ticks)
    { buttonEvent(press, DeviceId(), button, ticks); }
  ...
  inline void buttonReleaseAllAtDevice(DeviceId deviceId, TTimerTicks ticks) {
    touch(ticks);
    buttonHistory(deviceId)->released_all(m_ticks);
  }
  inline void buttonReleaseAllDefault(TTimerTicks ticks)
    { buttonReleaseAllAtDevice(DeviceId(), ticks); }
  inline void buttonReleaseAllAtAllDevices(TTimerTicks ticks) {
    touch(ticks);
    for(ButtonHistoryMap::const_iterator i = m_buttonHistories.begin(); i != m_buttonHistories.end(); ++i)
      i->second->released_all(m_ticks);
  }
  
  // check state of selected button at selected device
  ...
  // check state of selected button at default device
  ...
  // check state of selected button at any device
  ...
  // take holder of buttons states history
