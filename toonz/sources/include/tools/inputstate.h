...
public:
  ...
  inline explicit TKey() : key(), generic(), numPad() {}

  inline explicit TKey(Qt::Key key, bool generic = true, bool numPad = false)
      : key(key), generic(generic), numPad(numPad) {}

  inline bool operator==(const TKey &other) const {
    if (generic || other.generic) return is(other.key);
    return key == other.key && numPad == other.numPad;
  }

  inline bool is(Qt::Key key) const { return mapKey(this->key) == mapKey(key); }

  inline bool isModifier() const { return isModifier(key); }
  inline bool isNumber() const { return isNumber(key); }
...
public:
  ...
  TTimerTicks ticks() const { return m_ticks; }
  void touch(TTimerTicks ticks);

  inline const KeyHistory::Pointer &keyHistory() const { return m_keyHistory; }
  inline KeyState::Pointer keyState() const { return keyHistory()->current(); }

  inline void keyEvent(bool press, Key key, TTimerTicks ticks) {
    touch(ticks);
    keyHistory()->change(press, key, m_ticks);
  }
  ...
  inline bool isKeyPressed(Key key) const { return keyFind(key); }
  inline double howLongKeyPressed(Key key, TTimerTicks ticks,
  ...
  inline KeyState::Holder keyStateHolder(TTimerTicks ticks,
                                         double timeOffset = 0.0) const {
    return KeyState::Holder(keyState(), ticks, timeOffset);
  }
  inline KeyState::Holder keyStateHolder() const {
    return keyStateHolder(m_ticks);
  }
  inline KeyHistory::Holder keyHistoryHolder(TTimerTicks ticks,
                                             double timeOffset = 0.0) const {
  ...
  inline void buttonEvent(bool press, DeviceId deviceId, Button button,
                          TTimerTicks ticks) {
    touch(ticks);
    buttonHistory(deviceId)->change(press, button, m_ticks);
  }
  inline void buttonPress(DeviceId deviceId, Button button, TTimerTicks ticks) {
    buttonEvent(true, deviceId, button, ticks);
  }
  inline void buttonRelease(DeviceId deviceId, Button button,
                            TTimerTicks ticks) {
  ...
  inline double howLongButtonPressed(DeviceId deviceId, Button button,
                                     TTimerTicks ticks,
                                     double timeOffset = 0.0) {
    return ButtonState::Holder::howLongPressed(buttonFind(deviceId, button),
                                               ticks, timeOffset);
  }
  inline double howLongButtonPressed(DeviceId deviceId, Button button) {
  ...
  inline double howLongButtonPressedDefault(Button button, TTimerTicks ticks,
                                            double timeOffset = 0.0) {
  ...
  inline ButtonState::Pointer buttonFindAny(Button button) {
    DeviceId deviceId;
    return buttonFindAny(button, deviceId);
  }
  inline bool isButtonPressedAny(Button button) {
    return buttonFindAny(button);
  }
  inline double howLongButtonPressedAny(Button button, TTimerTicks ticks,
                                        double timeOffset = 0.0) {
    return ButtonState::Holder::howLongPressed(buttonFindAny(button), ticks,
                                               timeOffset);
  }
