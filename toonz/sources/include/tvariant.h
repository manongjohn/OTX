...
public:
  explicit TVariantSyntaxException(int row = 0, int col = 0,
                                   const std::string &msg = std::string())
      : TException(std::to_string(row) + ":" + std::to_string(col) + ": " +
                   msg) {}
};
...
public:
  inline explicit TVariantPathEntry(int index = -1) : m_index(index) {}
  ...
  inline bool isIndex() const { return m_index >= 0; }
  inline bool isField() const { return !isIndex(); }
  inline int index() const { return m_index; }
  inline TStringId field() const { return m_field; }
...
public:
  ...
  static int compare(const TVariantPath &a, int beginA, const TVariantPath &b,
                     int beginB, int count);
};
...
public:
  enum Type { None, Bool, Double, String, List, Map };
...
private:
  ...
  inline void setParent(TVariant &parent,
                        const TStringId &parentField = TStringId()) {
...
public:
  ...
  inline explicit TVariant(Type t)
      : m_type(t), m_bool(), m_double(), m_owner(), m_root(this), m_parent() {}
  ...
  inline void reset() { setType(None); }

  inline Type getType() const { return m_type; }
  inline bool isNone() const { return m_type == None; }
  ...
  inline const TVariantList &getList() const { return m_list; }
  inline const TVariantMap &getMap() const { return m_map; }

  inline void setNone() { reset(); }
  ...
  inline void clearList() { resize(0); }
  inline void append(const TVariant &v) { insert((int)m_list.size(), v); }
  ...
  inline const TVariant &operator[](const TVariantPathEntry &entry) const {
    return entry.isIndex() ? (m_type == List ? (*this)[entry.index()] : blank())
  ...
  inline TVariant &operator[](const TVariantPath &path) { return byPath(path); }

  // hierarchy methods
  inline const TVariantOwner *owner() const { return m_root->m_owner; }
  inline TVariantOwner *owner() { return m_root->m_owner; }
  inline const TVariant &root() const { return *m_root; }
  inline TVariant &root() { return *m_root; }
  inline const TVariant *parent() const { return m_parent; }
  inline TVariant *parent() { return m_parent; }
  inline int parentIndex() const {
    return m_parent || !m_parentField ? this - &m_parent->m_list.front() : 0;
  }
  inline const TStringId &parentField() const { return m_parentField; }
  inline bool isRoot() const { return this == m_root; }
  ...
  inline TVariantPathEntry parentPathEntry() const {
    return !m_parent
               ? TVariantPathEntry()
               : m_parent->m_type == Map
                     ? TVariantPathEntry(m_parentField)
                     : TVariantPathEntry(this - &m_parent->m_list.front());
  }
  inline int getParentPathSize() const { return getParentPathSize(*m_root); }
  ...
  bool getChildPathEntry(const TVariant &child,
                         TVariantPathEntry &outEntry) const;
  ...
  void fromStream(std::istream &stream, int *currentRow = 0,
                  int *currentCol = 0);

  std::string toString(bool pretty = false, int level = 0) const;
  void fromString(const std::string &str, int *currentRow = 0,
                  int *currentCol = 0);
};
