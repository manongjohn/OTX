...
bool TGuidelineLineBase::truncateRay(const TRectD &bounds, TPointD &p0,
  ...
  if (p0.x <= p1.x) {
    if (b.x0 < p0.x) b.x0 = p0.x;
  } else {
    if (b.x1 > p0.x) b.x1 = p0.x;
  }
  if (p0.y <= p1.y) {
    if (b.y0 < p0.y) b.y0 = p0.y;
  } else {
    if (b.y1 > p0.y) b.y1 = p0.y;
  }
...
void TGuidelineLineBase::drawLine(const TPointD &p0, const TPointD &p1,
  ...
  if (restrict0 && restrict1
          ? !truncateLine(oneBox, pp0, pp1)
...
TGuidelineRay::TGuidelineRay(bool enabled, double magnetism, const TPointD &p0,
                             const TPointD &p1)
    : TGuidelineLineBase(enabled, magnetism, p0, p1)
    , dir(calcDirection(p0, p1)) {}
