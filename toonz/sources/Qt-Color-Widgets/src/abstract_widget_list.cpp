...
AbstractWidgetList::AbstractWidgetList(QWidget *parent)
  ...
  connect(&p->mapper_up, SIGNAL(mapped(QWidget *)),
          SLOT(up_clicked(QWidget *)));
  connect(&p->mapper_down, SIGNAL(mapped(QWidget *)),
          SLOT(down_clicked(QWidget *)));
  connect(&p->mapper_remove, SIGNAL(mapped(QWidget *)),
          SLOT(remove_clicked(QWidget *)));
  ...
  QPushButton *add_button = new QPushButton(
      QIcon::fromTheme(QStringLiteral("list-add")), tr("Add New"));

  verticalLayout->addWidget(add_button);
  connect(add_button, &QAbstractButton::clicked, this,
          &AbstractWidgetList::append);
}

AbstractWidgetList::~AbstractWidgetList() { delete p; }

int AbstractWidgetList::count() const { return p->widgets.size(); }
...
void AbstractWidgetList::clear() {
  p->widgets.clear();
  while (p->table->rowCount() > 0) p->table->removeRow(0);
}
...
void AbstractWidgetList::appendWidget(QWidget *w) {
  ...
  QWidget *b_up =
      create_button(w, &p->mapper_up, QStringLiteral("go-up"), tr("Move Up"));
  QWidget *b_down = create_button(w, &p->mapper_down, QStringLiteral("go-down"),
                                  tr("Move Down"));
  QWidget *b_remove = create_button(
      w, &p->mapper_remove, QStringLiteral("list-remove"), tr("Remove"));
  if (row == 0)
...
QWidget *AbstractWidgetList::widget(int i) {
  if (isValidRow(i)) return p->widgets[i];
  return 0;
}
