...
public:
  ...
  qreal PixHue(float x, float y) {
    if (comp_x == Hue) return x;
    if (comp_y == Hue) return y;
    return hue;
  }

  qreal PixSat(float x, float y) {
    if (comp_x == Saturation) return x;
    if (comp_y == Saturation) return y;
    return sat;
  }

  qreal PixVal(float x, float y) {
    if (comp_x == Value) return x;
    if (comp_y == Value) return y;
    return val;
  }
  ...
  void renderSquare(const QSize& size) {
    ...
    for (int y = 0; y < size.height(); ++y) {
      ...
      for (int x = 0; x < size.width(); ++x) {
        ...
        square.setPixel(x, y, QColor::fromHsvF(PixHue(xfloat, yfloat),
  ...
  QPointF selectorPos(const QSize& size) {
    ...
    switch (comp_x) {
    case Hue:
      pt.setX(size.width() * hue);
      break;
    case Saturation:
      pt.setX(size.width() * sat);
      break;
    case Value:
      pt.setX(size.width() * val);
      break;
    }
    switch (comp_y) {
    case Hue:
      pt.setY(size.height() * (1 - hue));
      break;
    case Saturation:
      pt.setY(size.height() * (1 - sat));
      break;
    case Value:
      pt.setY(size.height() * (1 - val));
      break;
    }
    return pt;
  }
  ...
  void setColorFromPos(const QPoint& pt, const QSize& size) {
    QPointF ptfloat(qBound(0.0, qreal(pt.x()) / size.width(), 1.0),
...
Color2DSlider::~Color2DSlider() { delete p; }

QColor Color2DSlider::color() const {
  return QColor::fromHsvF(p->hue, p->sat, p->val);
}

QSize Color2DSlider::sizeHint() const { return {128, 128}; }

qreal Color2DSlider::hue() const { return p->hue; }

qreal Color2DSlider::saturation() const { return p->sat; }

qreal Color2DSlider::value() const { return p->val; }

Color2DSlider::Component Color2DSlider::componentX() const { return p->comp_x; }

Color2DSlider::Component Color2DSlider::componentY() const { return p->comp_y; }
