...
ColorDialog::ColorDialog(QWidget *parent, Qt::WindowFlags f)
  ...
  QPushButton *pickButton =
      p->ui.buttonBox->addButton(tr("Pick"), QDialogButtonBox::ActionRole);
  pickButton->setIcon(QIcon::fromTheme(QStringLiteral("color-picker")));

  setButtonMode(OkApplyCancel);

  connect(p->ui.wheel, &ColorWheel::displayFlagsChanged, this,
          &ColorDialog::wheelFlagsChanged);
}

QSize ColorDialog::sizeHint() const { return QSize(400, 0); }
...
QColor ColorDialog::color() const {
  ...
  if (p->alpha_enabled) col.setAlpha(p->ui.slide_alpha->value());
  return col;
}
...
bool ColorDialog::alphaEnabled() const { return p->alpha_enabled; }
...
void ColorDialog::update_widgets() {
  ...
  if (!p->ui.edit_hex->isModified()) p->ui.edit_hex->setColor(col);
...
void ColorDialog::set_hsv() {
  if (!signalsBlocked()) {
    p->ui.wheel->setColor(QColor::fromHsv(p->ui.slide_hue->value(),
...
void ColorDialog::set_rgb() {
  if (!signalsBlocked()) {
    QColor col(p->ui.slide_red->value(), p->ui.slide_green->value(),
