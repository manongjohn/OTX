...
public:
  ...
  void addPalette(ColorPalette& palette) {
    ...
    if (!palette.fileName().isEmpty()) {
      ...
      if (file.dir().canonicalPath() !=
          QDir(model->savePath()).canonicalPath()) {
  ...
  bool openPalette(const QString& file, int type) {
    if (type == 1) return openImage(file);
    return openGpl(file);
  }
};
...
ColorPaletteWidget::ColorPaletteWidget(QWidget* parent)
  ...
  connect(p->swatch, &Swatch::colorSizeChanged, this,
          &ColorPaletteWidget::colorSizeChanged);
  connect(p->swatch, &Swatch::colorSizePolicyChanged, this,
          &ColorPaletteWidget::colorSizePolicyChanged);
  connect(p->swatch, &Swatch::forcedRowsChanged, this,
          &ColorPaletteWidget::forcedRowsChanged);
  connect(p->swatch, &Swatch::forcedColumnsChanged, this,
          &ColorPaletteWidget::forcedColumnsChanged);
  connect(p->swatch, &Swatch::colorSelected, this,
          (void (ColorPaletteWidget::*)(const QColor&)) &
              ColorPaletteWidget::currentColorChanged);
  connect(p->swatch, &Swatch::selectedChanged, this,
          (void (ColorPaletteWidget::*)(int)) &
              ColorPaletteWidget::currentColorChanged);
  connect(p->swatch, &Swatch::borderChanged, this,
          &ColorPaletteWidget::borderChanged);

  connect(&p->swatch->palette(), &ColorPalette::dirtyChanged,
          p->button_palette_save, &QWidget::setEnabled);
  connect(&p->swatch->palette(), &ColorPalette::dirtyChanged,
          p->button_palette_revert, &QWidget::setEnabled);

  connect(p->palette_list,
          (void (QComboBox::*)(int)) & QComboBox::currentIndexChanged, this,
          &ColorPaletteWidget::currentRowChanged);
  ...
  connect(p->button_color_remove, &QAbstractButton::clicked, p->swatch,
          &Swatch::removeSelected);
  ...
  connect(p->button_palette_save, &QAbstractButton::clicked, [this]() {
    if (!p->read_only && p->hasSelectedPalette() &&
        p->swatch->palette().dirty())
      if (p->model->updatePalette(p->palette_list->currentIndex(),
                                  p->swatch->palette())) {
  ...
  connect(p->button_palette_duplicate, &QAbstractButton::clicked, [this]() {
    if (p->hasSelectedPalette()) {
      ...
      bool ok = false;
      QString name =
          QInputDialog::getText(this, tr("New Palette"), tr("Name"),
                                QLineEdit::Normal, new_palette.name(), &ok);
      if (ok) {
  ...
  connect(p->button_palette_new, &QAbstractButton::clicked, [this]() {
    if (p->hasSelectedPalette()) {
      bool ok      = false;
      QString name = QInputDialog::getText(this, tr("New Palette"), tr("Name"),
  ...
  connect(p->button_palette_open, &QAbstractButton::clicked,
          [this, image_formats]() {
            if (p->model) {
              QString default_dir;
              if (p->hasSelectedPalette()) {
              ...
              QStringList file_formats =
                  QStringList() << tr("GIMP Palettes (*.gpl)")
...
ColorPaletteModel* ColorPaletteWidget::model() const { return p->model; }

const ColorPalette& ColorPaletteWidget::currentPalette() const {
  return p->swatch->palette();
}

QSize ColorPaletteWidget::colorSize() const { return p->swatch->colorSize(); }

Swatch::ColorSizePolicy ColorPaletteWidget::colorSizePolicy() const {
  return p->swatch->colorSizePolicy();
}

QPen ColorPaletteWidget::border() const { return p->swatch->border(); }

int ColorPaletteWidget::forcedRows() const { return p->swatch->forcedRows(); }
int ColorPaletteWidget::forcedColumns() const {
  return p->swatch->forcedColumns();
}

bool ColorPaletteWidget::readOnly() const { return p->read_only; }
...
void ColorPaletteWidget::setColorSizePolicy(
    Swatch::ColorSizePolicy colorSizePolicy) {
...
void ColorPaletteWidget::setReadOnly(bool readOnly) {
  if (readOnly == p->read_only) return;
...
void ColorPaletteWidget::on_swatch_doubleClicked(int index) {
  if (!p->read_only) {
    ...
    if (dialog.exec()) p->swatch->palette().setColorAt(index, dialog.color());
  }
}
...
void ColorPaletteWidget::clearCurrentColor() { p->swatch->clearSelection(); }
