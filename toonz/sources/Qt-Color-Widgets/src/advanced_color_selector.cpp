...
public:
  ...
  void setColor(const QColor& color) { widget->setColor(color); }
  void setReadOnly(bool ro) { widget->setReadOnly(ro); }
  void setSelected(bool active) {
    setStyleSheet(active ? "font-weight: bold" : "");
  }
  bool isModified() const { return widget->isModified(); }
...
QAction* addEnableDisableAction(QMenu* menu, QWidget* widget, QString name) {
  ...
  QObject::connect(action, &QAction::toggled,
                   [widget](bool show) { widget->setVisible(show); });
...
public:
  CustomPalette(AdvancedColorSelector* parent) : Swatch(), parent(parent) {}
  ...
  bool event(QEvent* event) override {
    if (event->type() == QEvent::ToolTip) {
      ...
      if (indexAt(help_ev->pos()) == -1) {
        QToolTip::showText(help_ev->globalPos(),
                           tr("Left-click to add, right-click to delete, "
...
public:
  Private(AdvancedColorSelector* parent)
    ...
    auto harmony_none =
        newToolButton(QIcon(":/color_widgets/harmony/none.png"), [this]() {
          wheel->clearHarmonies();
          updateColors();
        });
    harmony_buttons->addButton(harmony_none);
    harmony_buttons->addButton(newToolButton(
        QIcon(":/color_widgets/harmony/complementary.png"), [this]() {
          wheel->clearHarmonies();
          wheel->addHarmony(0.5, false);
          updateColors();
        }));
    harmony_buttons->addButton(
        newToolButton(QIcon(":/color_widgets/harmony/analogus.png"), [this]() {
          wheel->clearHarmonies();
    ...
    harmony_buttons->addButton(
        newToolButton(QIcon(":/color_widgets/harmony/tetradic.png"), [this]() {
          wheel->clearHarmonies();
          wheel->addHarmony(0.5, false);
          auto a = wheel->addHarmony(0.125, true);
    ...
    connect(form_button, &QToolButton::toggled, [this](bool square) {
      if (square) {
        ...
        wheel->setDisplayFlags(ColorWheel::SHAPE_SQUARE |
                               ColorWheel::ANGLE_FIXED);
      } else {
        form_button->setIcon(QIcon(":/color_widgets/harmony/rectangle.png"));
        wheel->setDisplayFlags(ColorWheel::SHAPE_TRIANGLE |
                               ColorWheel::ANGLE_ROTATING);
      }
    });
    ...
    config_menu_button->setDefaultAction(
        new QAction(QIcon::fromTheme("configure"), "Configure"));
    ...
    auto enable_rgb_action =
        addEnableDisableAction(config_menu, rgb_chooser, "RGB sliders");
...
public:
  bool eventFilter(QObject* /*target*/, QEvent* event) override {
    if (event->type() == QEvent::Resize) {
      ...
      int i = 0;
      auto x_start =
          std::max(0.0, 0.95 * w - 40.0 * 3.5 - std::max(w - h, 0) / 2.0);
  ...
  void addColorWidget(QObject* widget) {
    ...
    connect(widget, SIGNAL(colorChanged(QColor)), parent,
            SLOT(setBaseColor(QColor)));
  }
  void addColorWidgetWOAlpha(QObject* widget) {
    widgets.push_back(widget);
    connect(widget, SIGNAL(colorChanged(QColor)), parent,
            SLOT(setBaseColorWOAlpha(QColor)));
  }
  void removeColorWidget(QObject* widget) {
    widgets.removeAll(widget);
    disconnect(widget, SIGNAL(colorChanged(QColor)), parent,
               SLOT(setBaseColor(QColor)));
    disconnect(widget, SIGNAL(colorChanged(QColor)), parent,
               SLOT(setBaseColorWOAlpha(QColor)));
  }
  ...
  void setColor(QColor c) {
    ...
    setBaseColor(
        QColor::fromHsvF(baseHue, c.saturationF(), c.valueF(), c.alphaF()));
  }
  ...
  void updateColors() {
    ...
    if (harmony_colors_layout == nullptr ||
        (unsigned)harmony_colors_layout->count() != count) {
    ...
    for (auto widget : harmony_colors_widgets) {
      if (!widget->isModified()) widget->setColor(colors[i]);
  ...
  void setHarmony(int i) {
    if (i < 0 || i >= (int)wheel->harmonyCount()) i = 0;
    selected_harmony                                = i;
    int j                                           = 0;
    for (auto widget : harmony_colors_widgets) {
  ...
  QColor baseColor() const { return wheel->color(); }
...
HarmonyButton::HarmonyButton(AdvancedColorSelector::Private* parent, unsigned n)
    : parent(parent), n(n), widget(new ColorLineEdit(this)) {
  ...
  if (n == 0) {
    connect(widget, &ColorLineEdit::colorChanged, parent,
            &AdvancedColorSelector::Private::setBaseColor);
    connect(widget, &ColorLineEdit::editingFinished,
...
void AdvancedColorSelector::saveToHistory() {
  ...
  if (h->palette().colorAt(0) == color()) return;
