...
ColorPalette::ColorPalette(const QVector<QColor>& colors, const QString& name,
                           int columns)
    : p(new Private) {
  setName(name);
  setColumns(columns);
...
ColorPalette::~ColorPalette() { delete p; }
...
int ColorPalette::count() const { return p->colors.size(); }

int ColorPalette::columns() { return p->columns; }

QString ColorPalette::name() const { return p->name; }
...
bool ColorPalette::save() {
  ...
  QFile file(filename);
  if (!file.open(QFile::Text | QFile::WriteOnly)) return false;
  ...
  for (int i = 0; i < p->colors.size(); i++) {
    stream << qSetFieldWidth(3) << p->colors[i].first.red() << qSetFieldWidth(0)
           << ' ' << qSetFieldWidth(3) << p->colors[i].first.green()
           << qSetFieldWidth(0) << ' ' << qSetFieldWidth(3)
           << p->colors[i].first.blue() << qSetFieldWidth(0) << '\t'
...
QString ColorPalette::fileName() const { return p->fileName; }
...
void ColorPalette::insertColor(int index, const QColor& color,
                               const QString& name) {
  if (index < 0 || index > p->colors.size()) return;
...
QPixmap ColorPalette::preview(const QSize& size,
                              const QColor& background) const {
  if (!size.isValid() || p->colors.empty()) return QPixmap();
...
bool ColorPalette::dirty() const { return p->dirty; }

void ColorPalette::setDirty(bool dirty) {
  if (dirty != p->dirty) Q_EMIT dirtyChanged(p->dirty = dirty);
}

QVector<QColor> ColorPalette::onlyColors() const {
  QVector<QColor> out;
  out.reserve(p->colors.size());
  for (int i = 0; i < p->colors.size(); i++) out.push_back(p->colors[i].first);
  return out;
}
