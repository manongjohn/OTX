...
class QCP_EXPORT ColorPalette : public QObject {
   ...
   */
  Q_PROPERTY(QVector<value_type> colors READ colors WRITE setColors NOTIFY
                 colorsChanged)
  /**
...
public:
  ...
  ColorPalette(const QVector<QColor>& colors, const QString& name = QString(),
               int columns = 0);
  ColorPalette(const QVector<QPair<QColor, QString>>& colors,
               const QString& name = QString(), int columns = 0);
   ...
   */
  QPixmap preview(const QSize& size,
...
public Q_SLOTS:
  ...
  /**
   * \brief save to file, the filename is \c fileName or determined
   * automatically
   * \returns \b true on success
   */
  bool save();
...
Q_SIGNALS:
  ...
  /**
   * \brief Emitted when the color or the name at the given index has been
   * modified
   */
  ...
  /**
   * \brief Emitted when the colors have been modified with a simple operation
   * (set, append etc.)
   */
...
private:
  ...
  /**
   * \brief Emit all the necessary signals when the palette has been completely
   * overwritten
   */
  void emitUpdate();
