...
public:
  ...
  void dropEvent(QDropEvent* event) {
    ...
    if (drop_index == -1) drop_index = palette.count();
  ...
  QSizeF actualColorSize() {
    ...
    if (!rowcols.isValid()) return QSizeF();
  ...
  QRectF indexRect(int index, const QSize& rowcols, const QSizeF& color_size) {
    ...
    return QRectF(index % rowcols.width() * color_size.width(),
  ...
  QRectF indexRect(int index) {
    QSize rc = rowcols();
    if (index == -1 || !rc.isValid()) return QRectF();
    return indexRect(index, rc, actualColorSize(rc));
  }
};

Swatch::Swatch(QWidget* parent) : QWidget(parent), p(new Private(this)) {
  connect(&p->palette, &ColorPalette::colorsChanged, this,
          &Swatch::paletteModified);
  connect(&p->palette, &ColorPalette::colorAdded, this,
          &Swatch::paletteModified);
  connect(&p->palette, &ColorPalette::colorRemoved, this,
          &Swatch::paletteModified);
  connect(&p->palette, &ColorPalette::columnsChanged, this,
          (void (QWidget::*)()) & QWidget::update);
  connect(&p->palette, &ColorPalette::colorsUpdated, this,
          (void (QWidget::*)()) & QWidget::update);
  connect(&p->palette, &ColorPalette::colorChanged, [this](int index) {
    if (index == p->selected) Q_EMIT colorSelected(p->palette.colorAt(index));
  });
...
Swatch::~Swatch() { delete p; }

QSize Swatch::sizeHint() const {
  QSize rowcols = p->rowcols();

  if (!p->color_size.isValid() || !rowcols.isValid()) return QSize();

  return QSize(p->color_size.width() * rowcols.width(),
               p->color_size.height() * rowcols.height());
}

QSize Swatch::minimumSizeHint() const {
  if (p->size_policy != Hint) return sizeHint();
  return QSize();
}

const ColorPalette& Swatch::palette() const { return p->palette; }

ColorPalette& Swatch::palette() { return p->palette; }

int Swatch::selected() const { return p->selected; }

QColor Swatch::selectedColor() const { return p->palette.colorAt(p->selected); }

int Swatch::indexAt(const QPoint& pt) {
  QSize rowcols = p->rowcols();
  if (rowcols.isEmpty()) return -1;
  ...
  if (index >= p->palette.count()) return -1;
  return index;
}
...
void Swatch::setSelected(int selected) {
  if (selected < 0 || selected >= p->palette.count()) selected = -1;

  if (selected != p->selected) {
    Q_EMIT selectedChanged(p->selected = selected);
    if (selected != -1) Q_EMIT colorSelected(p->palette.colorAt(p->selected));
    update();
  }
}

void Swatch::clearSelection() { setSelected(-1); }

void Swatch::paintEvent(QPaintEvent* event) {
  Q_UNUSED(event)
  QSize rowcols = p->rowcols();
  if (rowcols.isEmpty()) return;
...
void Swatch::keyPressEvent(QKeyEvent* event) {
  if (p->palette.count() == 0) QWidget::keyPressEvent(event);
  ...
  case Qt::Key_PageDown:
    if (selected == -1) {
      ...
      if (selected >= count) selected -= columns;
    }
    break;
  }
...
void Swatch::mousePressEvent(QMouseEvent* event) {
  if (event->button() == Qt::LeftButton) {
    ...
    if (index != -1) Q_EMIT rightClicked(index);
  }
}
...
void Swatch::mouseMoveEvent(QMouseEvent* event) {
  if (p->drag_index != -1 && (event->buttons() & Qt::LeftButton) &&
    ...
    if (!p->readonly) actions |= Qt::MoveAction;
    drag->exec(actions);
  }
}
...
void Swatch::mouseDoubleClickEvent(QMouseEvent* event) {
  if (event->button() == Qt::LeftButton) {
    int index = indexAt(event->pos());
    if (index != -1) Q_EMIT doubleClicked(index);
  }
}
...
void Swatch::dropEvent(QDropEvent* event) {
  ...
  // Not a color, discard
  if (!p->drop_color.isValid() || p->drop_index == -1) return;
  ...
  if (event->dropAction() == Qt::MoveAction && event->source() == this) {
    ...
    if (p->drop_index != p->drag_index && p->drop_index != p->drag_index + 1) {
      ...
      if (p->drop_index > p->drag_index) p->drop_index--;
...
void Swatch::paletteModified() {
  if (p->selected >= p->palette.count()) clearSelection();
...
QSize Swatch::colorSize() const { return p->color_size; }

QSizeF Swatch::actualColorSize() const { return p->actualColorSize(); }
...
int Swatch::forcedColumns() const { return p->forced_columns; }

int Swatch::forcedRows() const { return p->forced_rows; }

void Swatch::setForcedColumns(int forcedColumns) {
  if (forcedColumns <= 0) forcedColumns = 0;
...
bool Swatch::readOnly() const { return p->readonly; }
...
bool Swatch::event(QEvent* event) {
  if (event->type() == QEvent::ToolTip) {
    ...
    if (index != -1) {
      ...
      if (!name.isEmpty()) message = tr("%1 (%2)").arg(name).arg(message);
...
QPen Swatch::border() const { return p->border; }
