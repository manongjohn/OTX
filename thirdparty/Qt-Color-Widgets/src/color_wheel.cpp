...
struct RingEditor {
  ...
  int symmetric_to;
  int opposite_to;
  RingEditor(double hue_diff, bool editable, int symmetric_to = -1,
             int opposite_to = -1)
      : hue_diff(hue_diff)
      , editable(editable)
 ...
 */
inline double normalize(double angle) { return angle - std::floor(angle); }
...
public:
  ...
  QLineF line_to_point(const QPoint& p) const {
    return QLineF(w->geometry().width() / 2, w->geometry().height() / 2, p.x(),
                  p.y());
  }
  ...
  void render_square() {
    ...
    for (int y = 0; y < width; ++y) {
      for (int x = 0; x < width; ++x) {
        inner_selector.setPixel(x, y, color_from(hue, double(width - x) / width,
                                                 double(y) / width, 1)
                                          .rgb());
      }
    }
...
ColorWheel::~ColorWheel() { delete p; }
...
qreal ColorWheel::hue() const {
  if ((p->display_flags & COLOR_LCH) && p->sat > 0.01) return color().hueF();
  return p->hue;
}

qreal ColorWheel::saturation() const { return color().hsvSaturationF(); }

qreal ColorWheel::value() const { return color().valueF(); }

unsigned int ColorWheel::wheelWidth() const { return p->wheel_width; }
...
void ColorWheel::paintEvent(QPaintEvent*) {
  ...
  // hue wheel
  if (p->hue_ring.isNull()) p->render_ring();
  ...
  // lum-sat square
  if (p->inner_selector.isNull()) p->render_inner_selector();
...
void ColorWheel::mouseMoveEvent(QMouseEvent* ev) {
  if (p->mouse_status == DragCircle) {
    ...
    if (p->display_flags & SHAPE_SQUARE) {
      ...
      if (slice_h > 0) p->sat = qBound(0.0, (pt.y() - ymin) / slice_h, 1.0);
    }
  ...
  if (ev->buttons() & Qt::LeftButton) {
    ...
    if (ray.length() <= p->inner_radius())
      ...
      for (auto const& editor : p->ring_editors) {
        ...
        if (editor.editable && editor.hue_diff <= hue_diff + eps &&
...
void ColorWheel::setColor(QColor c) {
  ...
  qreal oldh = p->hue;
  p->set_color(c);
  if (!qFuzzyCompare(oldh + 1, p->hue + 1)) p->render_inner_selector();
  update();
...
void ColorWheel::setDisplayFlags(DisplayFlags flags) {
  if (!(flags & COLOR_FLAGS)) flags |= default_flags & COLOR_FLAGS;
  if (!(flags & ANGLE_FLAGS)) flags |= default_flags & ANGLE_FLAGS;
  if (!(flags & SHAPE_FLAGS)) flags |= default_flags & SHAPE_FLAGS;
...
void ColorWheel::setDefaultDisplayFlags(DisplayFlags flags) {
  if (!(flags & COLOR_FLAGS)) flags |= hard_default_flags & COLOR_FLAGS;
  if (!(flags & ANGLE_FLAGS)) flags |= hard_default_flags & ANGLE_FLAGS;
  if (!(flags & SHAPE_FLAGS)) flags |= hard_default_flags & SHAPE_FLAGS;
  default_flags = flags;
}
...
unsigned ColorWheel::addSymmetricHarmony(unsigned relative_to) {
  ...
  if (relative_to >= count)
    throw std::out_of_range(
        "incorrect call to addSymmetricHarmony: harmony number out of range");
  auto& relative        = p->ring_editors[relative_to];
  relative.symmetric_to = count;
  p->ring_editors.emplace_back(normalize(-relative.hue_diff), relative.editable,
                               relative_to, -1);
  p->apply_harmonies();
  return count;
}
...
unsigned ColorWheel::addOppositeHarmony(unsigned relative_to) {
  ...
  if (relative_to >= count)
    throw std::out_of_range(
        "incorrect call to addOppositeHarmony: harmony number out of range");
  auto& relative       = p->ring_editors[relative_to];
  relative.opposite_to = count;
  p->ring_editors.emplace_back(normalize(0.5 + relative.hue_diff),
                               relative.editable, -1, relative_to);
  p->apply_harmonies();
  return count;
}
