...
public:
  ...
  void setPalette(const QColor& color, ColorLineEdit* parent) {
    if (preview_color) {
      ...
      QColor text = detail::color_lumaF(color) > 0.5 || color.alphaF() < 0.2
                        ? Qt::black
                        : Qt::white;
      parent->setStyleSheet(QString("background-color: %1; color: %2;")
                                .arg(bg.name())
                                .arg(text.name()));
    }
  }
...
ColorLineEdit::ColorLineEdit(QWidget* parent)
  ...
  p->background.setTexture(
      QPixmap(QStringLiteral(":/color_widgets/alphaback.png")));
  setColor(Qt::white);
...
ColorLineEdit::~ColorLineEdit() { delete p; }

QColor ColorLineEdit::color() const { return p->color; }
...
bool ColorLineEdit::showAlpha() const { return p->show_alpha; }
...
void ColorLineEdit::dragEnterEvent(QDragEnterEvent* event) {
  ...
  if (event->mimeData()->hasColor() ||
      (event->mimeData()->hasText() &&
       color_widgets::colorFromString(event->mimeData()->text(), p->show_alpha)
           .isValid())) {
...
void ColorLineEdit::dropEvent(QDropEvent* event) {
  ...
  if (event->mimeData()->hasColor()) {
    ...
    QColor col = color_widgets::colorFromString(event->mimeData()->text(),
                                                p->show_alpha);
    if (col.isValid()) {
      setColor(col);
      event->accept();
...
bool ColorLineEdit::previewColor() const { return p->preview_color; }
...
void ColorLineEdit::paintEvent(QPaintEvent* event) {
  if (p->customAlpha()) {
    ...
    QRect r =
        style()->subElementRect(QStyle::SE_LineEditContents, &panel, nullptr);
