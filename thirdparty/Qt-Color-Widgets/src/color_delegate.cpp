...
ColorDelegate::ColorDelegate(QWidget *parent) : QAbstractItemDelegate(parent) {}
...
void ColorDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
  ...
  if (index.data().canConvert<QColor>()) {
    ...
    if (option.widget->isEnabled()) panel.state = QStyle::State_Enabled;
    panel.rect                                  = option.rect;
    panel.lineWidth                             = 2;
    panel.midLineWidth                          = 0;
    panel.state |= QStyle::State_Sunken;
    option.widget->style()->drawPrimitive(QStyle::PE_Frame, &panel, painter,
                                          nullptr);
    QRect r = option.widget->style()->subElementRect(QStyle::SE_FrameContents,
                                                     &panel, nullptr);
...
bool ColorDelegate::editorEvent(QEvent *event, QAbstractItemModel *model,
  ...
  if (event->type() == QEvent::MouseButtonRelease &&
      index.data().canConvert<QColor>()) {
    QMouseEvent *mouse_event = static_cast<QMouseEvent *>(event);

    if (mouse_event->button() == Qt::LeftButton &&
        (index.flags() & Qt::ItemIsEditable)) {
      ColorDialog *editor =
          new ColorDialog(const_cast<QWidget *>(option.widget));
