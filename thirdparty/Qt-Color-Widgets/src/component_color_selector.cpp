...
public:
  ComponentContainer(ComponentColorSelector* widget)
      : w(widget), layout(new QVBoxLayout()) {
  ...
  void addWidget(QSlider* slider) {
    ...
    QObject::connect(slider, &QSlider::valueChanged,
                     [this]() { Q_EMIT w->colorChanged(color()); });
  }
...
ComponentColorSelector::ComponentColorSelector(
    std::unique_ptr<ComponentContainer> container, QWidget* parent)
    : QWidget(parent), p(std::move(container)) {
  p->init();
}

ComponentColorSelector::~ComponentColorSelector() {}

QColor ComponentColorSelector::color() const { return p->color(); }

void ComponentColorSelector::setColor(QColor c) { p->setColor(c); }
...
public:
  ...
  void init() override { w->setLayout(layout); }

  QColor color() const override {
    return QColor(red_slider->value(), green_slider->value(),
                  blue_slider->value(), alpha);
  }
...
public:
  ...
  QColor color() const override {
    return QColor::fromHsv(hue_slider->value(), saturation_slider->value(),
                           value_slider->value(), alpha);
  }
