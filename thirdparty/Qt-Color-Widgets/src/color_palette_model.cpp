...
public:
  ...
  Private() : icon_size(32, 32) {}

  bool acceptable(const QModelIndex& index) const {
    return acceptable(index.row());
  }

  bool acceptable(int row) const { return row >= 0 && row <= palettes.count(); }
  ...
  bool save(ColorPalette& palette,
            const QString& suggested_filename = QString()) {
    // Attempt to save with the existing file names
    if (!suggested_filename.isEmpty() &&
        attemptSave(palette, suggested_filename))
      return true;
    if (attemptSave(palette, palette.fileName())) return true;

    // Set up the save directory
    QDir save_dir(save_path);
    if (!save_dir.exists() && !QDir().mkdir(save_path)) return false;
    ...
    // For all the files that match (Name)(Number).gpl, find the maximum
    // (Number)
    QRegularExpression name_regex(QRegularExpression::escape(palette.name()) +
                                  "([0-9]+)\\.gpl");
    int max = 0;
    ...
    return attemptSave(
        palette,
        save_dir.absoluteFilePath(
            QStringLiteral("%1%2.gpl").arg(palette.name()).arg(max + 1)));
  }
};

ColorPaletteModel::ColorPaletteModel() : p(new Private) {}

ColorPaletteModel::~ColorPaletteModel() { delete p; }

int ColorPaletteModel::rowCount(const QModelIndex&) const { return count(); }

QVariant ColorPaletteModel::data(const QModelIndex& index, int role) const {
  if (!p->acceptable(index)) return QVariant();
...
bool ColorPaletteModel::removeRows(int row, int count,
                                   const QModelIndex& parent) {
  Q_UNUSED(parent)

  if (!p->acceptable(row) || count <= 0) return false;

  auto begin = p->palettes.begin() + row;
  auto end =
      row + count >= p->palettes.size() ? p->palettes.end() : begin + count;
  for (auto it = begin; it != end; ++it) {
    if (!it->fileName().isEmpty()) {
      QFileInfo file(it->fileName());
      if (file.isWritable() && file.isFile()) QFile::remove(it->fileName());
    }
  }
...
QSize ColorPaletteModel::iconSize() const { return p->icon_size; }

void ColorPaletteModel::setIconSize(const QSize& iconSize) {
  if (p->icon_size != iconSize) Q_EMIT iconSizeChanged(p->icon_size = iconSize);
}

QString ColorPaletteModel::savePath() const { return p->save_path; }

QStringList ColorPaletteModel::searchPaths() const { return p->search_paths; }

void ColorPaletteModel::setSavePath(const QString& savePath) {
  if (p->save_path != savePath) Q_EMIT savePathChanged(p->save_path = savePath);
}
...
int ColorPaletteModel::count() const { return p->palettes.size(); }
...
bool ColorPaletteModel::updatePalette(int index, const ColorPalette& palette,
                                      bool save) {
  if (!p->acceptable(index)) return false;
  ...
  if (save) return p->save(local_palette, filename);
...
bool ColorPaletteModel::removePalette(int index, bool remove_file) {
  ...
  if (!file_name.isEmpty() && remove_file) {
    ...
    if (file.isWritable() && file.isFile()) return QFile::remove(file_name);
    return false;
  }
...
bool ColorPaletteModel::addPalette(const ColorPalette& palette, bool save) {
  ...
  if (save) return p->save(p->palettes.back());
